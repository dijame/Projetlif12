...
int CreeSocketServeur(const char* port) {
  ...
  int bon, res;
  // Des variable pour contenir des adresses de machine et des numeros de port afin de les afficher
  ...
  hints.ai_flags = AI_PASSIVE;    /* Signifie que toutes les addresses de la machine seront utilisées */
  ...
  while (rp != NULL) { // on parcourt la liste
    ...
    // partie optionnelle pour éviter d'etre rejeté par le système si le précédant test a planté
